...
import NfcDeepLinkScreen from '../../src/features/nfc/NfcDeepLinkScreen';
import useStore from '../../src/store/store';
import { hasAppAccess } from '../../src/auth/requireAuth';

export default function NfcTagRoute() {
  const profile = useStore((state: any) => state.profile);
  const authUserId = useStore((state: any) => state.authUserId);
  const params = useLocalSearchParams<{ tagCode?: string }>();
  const tagCode = Array.isArray(params.tagCode) ? params.tagCode[0] : params.tagCode;
